...
<https://github.com/voicetel/csharp-sdk/security/advisories/new>).
...
- The SDK uses `HttpClient` with `HttpClientHandler.AutomaticDecompression`
  enabled and the .NET default TLS posture (TLS 1.2+, OS-level session
  cache). If you supply a custom `HttpMessageHandler`, you are
...
Out of scope: vulnerabilities in the `VoiceTel.Sdk` NuGet package
